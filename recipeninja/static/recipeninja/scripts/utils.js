...
export async function loadRecipes(cuisines, query) {
    const cuisineParam = cuisines ? (
                            cuisines.length > 0 ? cuisines.join(',') : 'all'
                        ) : 'all';

    try {
        const response = await fetch(`/load_recipes/${cuisineParam}${query ? ( query.trim() ? `/${query}` : '' ) : ''}`);
...
export async function loadUserRecipes(username, getSaved=false) {
    try {
        const response = await fetch(`/load_user_recipes/${username}/${getSaved ? 1 : 0}`);
        const result = await response.json();
...
export async function deleteRecipe(recipeId) {
    try {
        // Await response
        const response = await fetch(`/delete_recipe/${recipeId}`, { method: 'DELETE' });
        const result = await response.json();
...
export function createRecipeBox(recipe, currentUsername, addDelete=false, isSavedRecipes=false) {
    // Destructure recipe
    const { id, name, poster, image, description, difficulty, duration, likers, savers } = recipe;
    ...
    const tagsElement = $('<div>')
        ...
        .html(`<div class="recipe-tag-item"><span class="recipe-tag-text">${duration}</span></div>`)
        .append(`<div class="recipe-tag-item"><span class="recipe-tag-text">${difficulty}</span></div>`);
    ...
    // Create like button
    const likeButtonElement = $(likedByUser ? (`
    ...
    // Create save element
    const saveButtonElement = $(savedByUser ? (`
...
function confirmDeleteCarouselRecipe(recipeId, recipeName) {
    ...
    $('#confirmDeleteRecipeButton').click(async () => {
        try {
            ...
            $('#recipesCount').text(`${prevCount - 1} recipe${prevCount - 1 > 1 ? 's' : ''}`);
